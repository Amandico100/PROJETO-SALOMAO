/**
 * 📤 ShareButton
 * Botão de compartilhamento no WhatsApp
 */

import { motion } from 'framer-motion';
import { cn } from '@/lib/utils';

interface Props {
    text?: string;
    shareMessage?: string;
    shareUrl?: string;
    className?: string;
}

export function ShareButton({
    text = 'COMPARTILHAR NO WHATSAPP',
    shareMessage = 'Acabei de fazer uma análise gratuita de segurança no Casa Segura. Achei muito interessante!',
    shareUrl,
    className
}: Props) {

    const handleShare = () => {
        const url = shareUrl || window.location.href;
        const message = `${shareMessage}\n\nVeja: ${url}`;
        const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(message)}`;
        window.open(whatsappUrl, '_blank');
    };

    return (
        <motion.button
            className={cn(
                'w-full py-4 px-6 rounded-xl font-bold',
                'bg-gradient-to-r from-green-600 to-green-500',
                'hover:from-green-500 hover:to-green-400',
                'flex items-center justify-center gap-3',
                'transition-all duration-300',
                'shadow-lg shadow-green-500/25',
                className
            )}
            onClick={handleShare}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
        >
            {/* Ícone WhatsApp */}
            <svg
                className="w-6 h-6"
                fill="currentColor"
                viewBox="0 0 24 24"
            >
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
            </svg>
            <span>{text}</span>
        </motion.button>
    );
}

// ============================================================================
// CONFIRMATION CARD COM SHARE
// ============================================================================

interface ConfirmationCardProps {
    title?: string;
    details: {
        slot: string;
        city: string;
        whatsapp: string;
    };
    shareEnabled?: boolean;
}

export function ConfirmationCard({
    title = 'Análise Agendada!',
    details,
    shareEnabled = true
}: ConfirmationCardProps) {
    return (
        <div className="min-h-screen flex flex-col pt-20 pb-8 px-4">
            <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
                {/* Ícone de sucesso */}
                <motion.div
                    className="w-24 h-24 bg-green-500/20 rounded-full flex items-center justify-center mb-6"
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    transition={{ type: 'spring', delay: 0.2 }}
                >
                    <span className="text-5xl">✅</span>
                </motion.div>

                {/* Título */}
                <motion.h2
                    className="text-3xl font-bold text-white text-center mb-6"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.3 }}
                >
                    {title}
                </motion.h2>

                {/* Detalhes */}
                <motion.div
                    className="w-full bg-white/5 rounded-2xl p-6 mb-8 border border-white/10"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.4 }}
                >
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            <span className="text-2xl">📅</span>
                            <div>
                                <p className="text-gray-400 text-sm">Turno</p>
                                <p className="text-white font-medium">{details.slot}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <span className="text-2xl">📍</span>
                            <div>
                                <p className="text-gray-400 text-sm">Cidade</p>
                                <p className="text-white font-medium">{details.city}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <span className="text-2xl">📞</span>
                            <div>
                                <p className="text-gray-400 text-sm">Contato</p>
                                <p className="text-white font-medium">{details.whatsapp}</p>
                            </div>
                        </div>
                    </div>
                </motion.div>

                {/* Mensagem */}
                <motion.p
                    className="text-gray-400 text-center mb-8"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ delay: 0.5 }}
                >
                    Em breve um Especialista entrará em contato.
                </motion.p>

                {/* Compartilhar */}
                {shareEnabled && (
                    <motion.div
                        className="w-full space-y-4"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: 0.6 }}
                    >
                        <div className="border-t border-white/10 pt-6">
                            <h3 className="text-lg font-bold text-white text-center mb-2">
                                Conhece alguém que também precisa?
                            </h3>
                            <p className="text-gray-400 text-sm text-center mb-4">
                                Indique um amigo ou familiar. A análise é 100% gratuita.
                            </p>
                            <ShareButton />
                        </div>
                    </motion.div>
                )}
            </div>
        </div>
    );
}
